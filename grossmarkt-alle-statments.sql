...
SELECT `name`, land
FROM kunden
ORDER BY land, `name` DESC;
# ACHTUNG! Default ist ASC (fÃ¼r land), am besten immer dahiner schreiben
...
FROM artikel
LIMIT 0, 10;
# zeigt von AB 0 - WEITERE 10 an oder AB 10, WEIETE 10
...
FROM bestellungen
WHERE YEAR(bestelldatum) = 1995
  AND QUARTER(bestelldatum) = 2;

# Wochentage ausgeben
SELECT WEEKDAY('1985-09-24');
# Mo -> 0, Di -> 1 ...
...
WHERE kundennr NOT IN (SELECT DISTINCT kundennr
                       FROM bestellungen);
# im subsatement here suche alle Kundennummern aus Bestellungen und lege mit DISTINCT fest, dass sie einzigartig sein mÃ¼ssen.
...
WHERE year(bestellungen.Bestelldatum) = 1995
  AND kunden.Name = 'around the horn';
...
FROM positionen
WHERE bestellnr IN
      (SELECT bestellnr # bei denen die BestellNr in Bestellungen vorkommt, bei denen wiederrum die Kundennummer KNR entspricht -> ist wie ein JOIN
       FROM bestellungen
       WHERE kundennr = @KNR);
...
SELECT *
FROM artikel
WHERE artikel.Lagerbestand < mindestbestand;
...
# ein neuer Kunde, Fritz Mustermann, möchte heute 2 Artikel bestellen
# 3 mal den Artikel Chang und 2 mal den Artikel Chai
# versendet wird die Bestellung von United Package
# und Janet Fuller nimmt die Bestellung auf
# tragen Sie die Daten in die DB ein
# Den Mitarbeiter über Subselect oder Variable eintragen

INSERT INTO kunden(Land, `Name`)
VALUES ('Deutschland', 'Fritz Mustermann');
SELECT @KID := LAST_INSERT_ID();

INSERT INTO bestellungen(KundenNr, MitarbeiterNr, VersandFirmenNr, Bestelldatum)
VALUES (@KID,
        (SELECT MitarbeiterNr FROM mitarbeiter WHERE Vorname = 'Janet'),
        (SELECT VersandFirmenNr FROM versandfirmen WHERE `Name` = 'United Package'),
        CURDATE());

INSERT INTO positionen(BestellNr, ArtikelNr, Preis, Anzahl, Rabatt)
VALUES (@KID,
        (SELECT ArtikelNr FROM artikel WHERE `Name` = 'Chang'),
        (SELECT Preis FROM artikel WHERE `Name` = 'Chang'),
        3,
        0),
       (@KID,
        (SELECT ArtikelNr FROM artikel WHERE `Name` = 'Chai'),
...
INSERT INTO positionen(BestellNr, ArtikelNr, Preis, Anzahl, Rabatt)
SELECT @BID, artikel.ArtikelNr, artikel.Preis, 3, 0
FROM artikel
WHERE name = 'chang';
